...
const Products: React.FC = () => {
  ...
  const [formData, setFormData] = useState<CreateProductData & { isActive?: boolean }>({
    name: '',
    sku: '',
    brand: '',
    description: '',
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      // Ensure isActive is explicitly included as boolean
      const updateData = {
        ...formData,
        isActive: formData.isActive !== undefined ? Boolean(formData.isActive) : true,
      };
      await dispatch(updateProduct({ id: selectedProduct.id, data: updateData })).unwrap();
        ...
        <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="w-full px-4 py-2.5 border border-[#E0E7F1] rounded-lg bg-gray-50 text-[#1A1F36]">
                <div className="font-medium">{categoryName || 'N/A'}</div>
                {formData.categoryId && (
                  <div className="text-xs text-[#4A5568] mt-1">ID: {formData.categoryId}</div>
                )}
              </div>
            </div>
            <div>
              ...
              <div className="w-full px-4 py-2.5 border border-[#E0E7F1] rounded-lg bg-gray-50 text-[#1A1F36]">
                <div className="font-medium">{subCategoryName || 'N/A'}</div>
                {formData.subCategoryId && (
                  <div className="text-xs text-[#4A5568] mt-1">ID: {formData.subCategoryId}</div>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  const isActiveValue = e.target.value === 'active';
                  setFormData({ ...formData, isActive: isActiveValue });
                }}
                className={`w-full px-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#007BFF] focus:border-transparent text-sm font-medium ${
                  formData.isActive === true
                    ? 'bg-green-50 border-green-200 text-green-700'
                    : 'bg-red-50 border-red-200 text-red-700'
                }`}
              >
                <option value="active" className="bg-white text-[#1A1F36]">Active</option>
                <option value="inactive" className="bg-white text-[#1A1F36]">Inactive</option>
              </select>
            </div>
          </div>
