export const formatTimestamp = (timestamp?: { seconds: number; nanoseconds: number }, showTime: boolean = false): string => {
...
export const formatMessageTime = (timestamp?: { seconds: number; nanoseconds: number }): string => {
    if (!timestamp) return '';
